...
export PS1='`if [ $? = 0 ]; then echo 😃; else echo 😓; fi` @\t [\w] \[\033[32m\]`git branch 2>/dev/null | grep ^* | sed "s/^\* \(.*\)$/on branch \1/g"`\[\033[31m\]`git status 2>/dev/null | grep "git add" | head -1 | sed "s/.*/*/g"`\n\[\033[m\]\n\u@\h`if [ "$(id -u)" = "0" ]; then echo "# "; else echo "$ "; fi`'
